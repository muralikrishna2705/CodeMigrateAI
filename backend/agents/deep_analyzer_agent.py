...
from models.schemas import DeepAnalysis
...
class DeepAnalyzerAgent(BaseAgent):
    ...
    async def run(self, state: MigrationState) -> AgentResult:
        ...
        # Analysis is a lightweight, structured task — routed to the fast model.
        # The output shape is carried by the schema rather than spelled out in
        # the prompt, so the two can no longer disagree.
